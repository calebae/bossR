...
```{r} 
image(trt.nifti1, z = 91, plot.type='single')
```
...
for(i in 1:Z){
  ...
  for(j in 1:nrow(tmp.dat)){
    ...
    Z.pixel <- (tmp.dat$Z[j]-2) : (tmp.dat$Z[j]+2)
...
```{r}
overlay.nifti(trt.nifti1, mask.dat, z = 91, plot.type = 'single')
```
...
```{r}
image(as.nifti(roi.img), z = 91, plot.type='single')
```

```{r}
overlay.nifti(as.nifti(roi.img), as.nifti(mask.dat[,,1:110]), z = 91, plot.type = 'single')
```
